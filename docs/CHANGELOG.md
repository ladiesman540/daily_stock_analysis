...
- [新功能] 盘中市场数据刷新：`--serve` 模式 30 分钟 tick 的非到期分支新增 `_intraday_market_refresh()`，在美股交易时段 (Mon-Fri 06:00-14:00 本地时) 保持 regime/portfolio/rotation 数据新鲜（INTRADAY_REFRESH_ENABLED 默认开启）。
- [新功能] 新增盘中阈值告警层：`send_threshold_alerts()` 在既有 regime/VIX/周期/轮动/纸面告警之外追加三类检查（逐项 fail-open，均复用 data/alert_state.json 去重）—— 高确信新想法（最新 discovery 快照需为当前美股交易日，composite ≥ DISCOVERY_ALERT_MIN_COMPOSITE(90) 且 candidate_score ≥ DISCOVERY_ALERT_MIN_CANDIDATE(70) 且不在上一快照合格集内，按 composite 取最高 3 个/日，单符号 30 天内不重复告警）、watchlist 异动（仅读 stock_daily 缓存且末根 K 线须为当前交易日，单日涨跌幅 ≥ WATCHLIST_MOVER_ALERT_PCT(5.0)% 或量比 ≥ WATCHLIST_MOVER_VOLUME_RATIO(2.5)（对比此前 20 日均量），全部异动合并为一条消息、每符号每交易日至多一次）、下跌日触发（最新 down_day 快照 triggered 且为当前交易日时每个 as_of 告警一次，附扛跌标的 Top 5）；状态文件中按符号记日期的去重字典保存时自动清理超 60 天条目；`--serve` 模式 30 分钟 tick 的非到期分支新增 `_intraday_threshold_alerts()`（ALERTS_INTRADAY_ENABLED=true 默认开启），告警发送与每日 notify 步骤解耦（见 `.env.example`）。
- [新功能] discovery 每日扫描新增两个独立观察筛选（watch flags，不参与动量综合分排名）：quiet_accumulation（近 5 个交易日均量 ≥ 1.5x 此前 20 个交易日均量、且收盘价较 5 个交易日前变动 ≤ ±3%、需 ≥26 根日线，新增暴露 quiet_accum_volume_ratio 字段；阈值可经 QUIET_ACCUM_VOL_RATIO / QUIET_ACCUM_WINDOW / QUIET_ACCUM_MAX_ABS_CHG_PCT 调整）与 beaten_down_reversal（收盘 ≤ 52 周高点的 60% 且 1M 相对 SPY 强度 > 0 且当日量比 ≥ 1.25x；可经 REVERSAL_MAX_PCT_OF_HIGH / REVERSAL_MIN_RS_1M / REVERSAL_MIN_VOLUME_RATIO 调整），两者对缺失输入均 fail-safe 为 False（见 `.env.example`）；GrindingLeaderScorer 评分范围改为综合分 Top 40 ∪ 新筛选命中行（命中行即使排名靠后也带 candidate_score/checklist 语言），reason 一行理由新增对应描述；记分卡 screens_json 经 SCREEN_KEYS 自动纳入两个新键（by_screen 细分自动出现），daily-brief discovery 节随行携带两个布尔（Telegram 摘要逐字节不变，回归测试覆盖）；Web 端 Signals/Today/Scorecard 徽章新增 "Quiet accumulation" / "Reversal setup"，glossary 新增 screen_quiet_accumulation / screen_beaten_down_reversal 条目。已知限制：Today 页 Top 5 仍按动量综合分排名，新筛选命中标的在 Signals 表与 Scorecard 处露出。
- [新功能] 新增记分卡前端界面：Web 新增 `/scorecard` 页（真实 flags 命中率头条 + flagged vs universe baseline 的 edge 框架、SIMULATED — backtested 独立区块（含固定幸存者偏差披露文案）、无模拟数据时空态卡内置 Run bootstrap 按钮（started/already_running/error 三态，约 10-15 分钟后刷新）、按筛选项命中率横向条形图（含 n）、综合分带/月度细分表、近期 flags 表（TickerChip + 筛选项徽章 + 状态徽章（expired 带 InfoTip）+ 状态与 real/simulated 过滤 + 分页）），侧边栏 Backtest 旁新增 Scorecard 项；Today 页新增 "10 · Scorecard / Is the system any good?" 紧凑卡（直接读取 daily-brief，无额外请求）；daily-brief 新增 `scorecard` 节（fail-open，as_of 取最新真实 flag 日期，real/simulated 精简统计永不合并），Telegram 摘要输出保持逐字节不变（回归测试覆盖）；glossary 新增 hit_rate 条目。
- [新功能] 新增记分卡回溯引导（retroactive bootstrap，模拟历史）：`ScorecardService.run_bootstrap()` 对当前 discovery universe（+SPY）做一次深度抓取（默认 600 个日历日、1800 秒超时、写穿 stock_daily 缓存故重跑近乎免费），按每第 5 个 SPY 交易日采样回溯约 39 周的 as-of 日期（严格早于最早的真实 discovery 快照，绝不写入 DiscoveryDaily），逐日期经单一截断卡点 `_bars_until()`（防前视偏差，独立单测）切片后重放现有筛选/百分位/综合分/GrindingLeaderScorer（sector_tailwind 在模拟中恒为 False，已在 caveats 披露），入选行用全量 bars 内存内前向评级后以 simulated=True 入库（与 real 永不合并，存在幸存者偏差上限）；新增本地脚本 `scripts/scorecard_bootstrap.py`（--weeks/--deep-days/--symbol-limit/--dry-run，dry-run 仅读缓存 SPY 交易日、零网络）与 API `POST /api/v1/research/scorecard/bootstrap`（守护线程 + 模块级锁，返回 started|already_running）；周数与抓取深度可经 SCORECARD_BOOTSTRAP_WEEKS / SCORECARD_BOOTSTRAP_DEEP_DAYS 调整（见 `.env.example`）。
- [新功能] 新增发现命中率记分卡（hit-rate scorecard）后端：每日快照新增 `scorecard` 步骤（discovery 之后），把历史 discovery 入选标的逐一评级 —— 90 个日历日窗口内收盘涨幅率先触及 +20% 记 hit、率先触及 -20% 记 flop、窗口结束（+7 天宽限）两者均未触及记 expired、窗口未结束记 open（命中率 = hits/(hits+flops+expired)，open 不计入）；新增 `discovery_flag_outcomes` 表与 DAO（(as_of, symbol) 唯一、screens_json 记录通过的筛选项、批量 upsert 保留 created_at、real 与 simulated 永不合并），对掉出每日抓取集的未决标的做受限补抓（上限 SCORECARD_FETCH_LIMIT=300、600 秒超时、fail-open）；新增 API `GET /api/v1/research/scorecard`（总体 + by_screen/by_score_band/by_month 细分 + caveats，空库返回 missing）、`GET /api/v1/research/scorecard/flags`（status/simulated/screen/days 过滤 + 分页）、`POST /api/v1/research/scorecard/run`；窗口与阈值可经 SCORECARD_* 环境变量调整（见 `.env.example`）。
...
- [改进] 记分卡摘要（`GET /api/v1/research/scorecard`）按人群拆分：real/simulated 各拆为 `flagged`（通过 ≥1 个筛选项或进入打分 top-40，作为头条命中率）与 `baseline`（全部流动性达标标的，刻意保留作对照组的市场基准命中率）两个子块（字段 flags/hits/flops/expired/open/decided/batting_average/avg_days_to_hit/avg_max_gain_pct），并新增 `edge` = flagged 与 baseline 命中率之差（任一为空时为 null）；by_screen/by_score_band/by_month 仅按 flagged 行统计；另修复 lookback 下限为 窗口+宽限+1 天，防止过小的 SCORECARD_LOOKBACK_DAYS 使 91-97 天的未决标的永远滞留在评估窗口之外。
