...
export const IntelligentImport: React.FC<IntelligentImportProps> = ({
  ...
  const handleImageFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        let fallback = 'Recognition failed. Try again.';
  ...
  const handleDataFile = useCallback(
    async (file: File) => {
      if (file.size > FILE_MAX) {
        setError('Files must be 2MB or smaller');
        return;
      }
      setError(null);
      setIsLoading(true);
  ...
  const handlePasteParse = useCallback(() => {
    ...
    if (new Blob([t]).size > TEXT_MAX) {
      setError('Pasted text must be 100KB or smaller');
      return;
    }
    setError(null);
    setIsLoading(true);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <textarea
            placeholder="Or paste copied CSV/Excel text..."
      ...
      {items.length > 0 && (
        <div className="space-y-2">
          <InlineAlert
            variant="warning"
            message="Review each item before merging. High-confidence items are checked by default; medium and low confidence need manual confirmation."
            className="rounded-xl px-3 py-2 text-xs shadow-none"
          />
          <div className="flex items-center justify-between">
            <span className="text-xs text-secondary-text">
              {validCount} mergeable items, {checkedCount} selected
            </span>
